...
// src/typography/generateTypographyVariant.ts
  ...
  components: {
    ...
    MuiListItemButton: {
      styleOverrides: {
        root: {
          borderRadius: 10
          // aligns with shape but not pill
        }
      }
    },
    MuiListItemIcon: {
      styleOverrides: {
        root: {
          minWidth: 36
          // tighter menu icon column; feels better in drawers
        }
      }
    }
  }
};
function createSiteTheme(opts = {}) {
  const { preset, overrides, responsiveFactor = 2.4 } = opts;
  let theme = styles.createTheme(baseThemeOptions, preset ?? {}, overrides ?? {});
  if (responsiveFactor !== false) {
    theme = styles.responsiveFontSizes(theme, { factor: responsiveFactor });
  }
  return theme;
}
...
exports.generateTypographyVariant = generateTypographyVariant;
